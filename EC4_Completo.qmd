...
```{r}
#Cantidad de restaurantes en cada dataset
num_restaurantes_train <- nrow(df_train)
num_restaurantes_test <- nrow(df_test)
total_restaurantes <- num_restaurantes_train + num_restaurantes_test
```

```{r}
cat("Cantidad de restaurantes en el dataset de entrenamiento:", num_restaurantes_train, "\n")
cat("Cantidad de restaurantes en el dataset de prueba:", num_restaurantes_test, "\n")
cat("Cantidad total de restaurantes:", total_restaurantes, "\n")
```

**Tiempo de Funcionamiento**

```{r}
# Crear categorías de funcionamiento basadas en el número de reseñas
df_train$funcionamiento_estimado <- cut(df_train$review_count, 
                                        breaks = c(-Inf, 50, 150, 300, Inf), 
                                        labels = c("Nuevo", "Intermedio", "Establecido", "Popular"))
# Tabla de resumen
table(df_train$funcionamiento_estimado)
```


Utilizamos el número de reseñas (review_count) como un indicador indirecto del tiempo de funcionamiento, bajo la suposición de que los restaurantes con más reseñas probablemente llevan más tiempo operando. El código divide las reseñas en cuatro categorías (Nuevo, Intermedio, Establecido, Popular) usando la función cut. Este enfoque permite clasificar a los restaurantes en rangos de funcionamiento estimado cuando no se dispone de fechas de apertura exactas. Aunque no es una medida precisa, proporciona una idea relativa del tiempo en operación.
...
# Calcular el promedio de reseñas para todos los restaurantes
avg_reviews <- mean(df_train$review_count, na.rm = TRUE)
cat("Promedio de reseñas (review_count) para todos los restaurantes:", avg_reviews, "\n")

# Agrupar por calificación de estrellas y calcular el promedio de reseñas por cada nivel de estrellas
activity_by_stars <- df_train %>%
  group_by(stars) %>%
  summarise(avg_reviews = mean(review_count, na.rm = TRUE))

# Mostrar los resultados
print("Promedio de reseñas agrupado por calificación de estrellas:")
print(activity_by_stars)
```
**Promedio de Reseñas General:** El promedio general de reseñas (review_count) de 173.5 para todos los restaurantes indica una medida de participación de los usuarios en Yelp, lo cual puede relacionarse con la popularidad y, de manera indirecta, con el nivel de ingresos.

**Promedio de Reseñas por Estrellas:** Al observar cómo el promedio de reseñas varía por calificación, se puede inferir que restaurantes con calificaciones de 3 a 4.5 estrellas tienen un promedio más alto de reseñas, lo que podría indicar mayor popularidad y, en teoría, mejores ganancias.

1.3 Explique porque una buena evaluación ayuda al restaurante.


*Una buena evaluación es crucial para un restaurante porque influye directamente en su reputación y visibilidad en plataformas como Yelp. Las evaluaciones positivas, reflejadas en un alto número de estrellas y reseñas favorables, generan confianza en los clientes potenciales, lo que aumenta las probabilidades de que elijan ese restaurante en lugar de otros. Además:*

**Atrae Nuevos Clientes:** Las personas suelen confiar en las opiniones de otros clientes al momento de decidir dónde comer. Un restaurante con buenas evaluaciones se percibe como confiable y de calidad, lo que atrae a nuevos clientes.

**Incrementa la Visibilidad:** En plataformas como Yelp, los restaurantes con mejores calificaciones tienden a aparecer en los primeros resultados de búsqueda. Esto aumenta la visibilidad del negocio y facilita que los clientes potenciales lo encuentren.

**Fomenta la Lealtad:** Los clientes satisfechos son más propensos a volver y a recomendar el restaurante a otros, creando una base de clientes leales que contribuye a la estabilidad del negocio a largo plazo.

**Impacto en las Ganancias:** Un restaurante con buenas evaluaciones generalmente experimenta un mayor flujo de clientes, lo que puede traducirse en mayores ingresos. Al ser visto como un lugar de calidad, las personas están dispuestas a gastar más, lo que impacta positivamente en las ganancias.
...
-   **BikeParking**: Estacionamiento para bicicletas

-   **ByAppointmentOnly**: Solo con cita previa

-   **WheelechairAccessible**: Accesible para sillas de ruedas

-   **OutdoorSeating**: Asientos al aire libre

-   **RestaurantsReservations**: Permite reservas

-   **DogsAllowed**: Permite entrada de perros

-   **Caters**: Servicio de catering
...
```{r}
# Reemplazar "df_train" con el nombre de tu dataframe
missing_percent <- sapply(df_train, function(x) {
  # Calcular el porcentaje de valores "Missing" en cada columna
  mean(x == "(Missing)") * 100
})

# Convertir a data frame y mostrar el resultado
...
```{r}
# Ver la estructura de todas las variables
str(df_train)

# ver la clase de cada variable
sapply(df_train, class)
```
```{r}
# Convertir una columna específica a character
df_train$Alcohol <- as.character(df_train$Alcohol)

# O convertir todas las columnas factor a character
df_train[] <- lapply(df_train, function(x) if(is.factor(x)) as.character(x) else x)
```


```{r}
# Obtener los nombres de las variables de tipo character
variables_character <- names(df_train)[sapply(df_train, is.character)]

# Obtener los nombres de las variables de tipo numeric
variables_numeric <- names(df_train)[sapply(df_train, is.numeric)]

# Mostrar los resultados
cat("Variables de tipo character:\n")
print(variables_character)

cat("\nVariables de tipo numeric:\n")
...
```{r}
# Lista de las 11 variables que deseas convertir
selected_vars <- c("GoodForKids", "Alcohol", "BusinessAcceptsCreditCards", "WiFi", 
                   "BikeParking", "ByAppointmentOnly", "WheelechairAccessible", 
                   "OutdoorSeating", "RestaurantsReservations", "DogsAllowed", "Caters")

# Convertir las 11 variables a factores con "(Missing)" como referencia en df_train
df_train[selected_vars] <- lapply(df_train[selected_vars], function(x) {
  factor(x, levels = unique(c("(Missing)", as.character(x)))) %>% 
    relevel(ref = "(Missing)")
})

# Convertir las 11 variables a factores con "(Missing)" como referencia en df_test
df_test[selected_vars] <- lapply(df_test[selected_vars], function(x) {
  factor(x, levels = unique(c("(Missing)", as.character(x)))) %>% 
    relevel(ref = "(Missing)")
})
...
```{r}
modelLR <- lm(stars ~ ., data = df_train)# TODOrevisar
summary(modelLR)
```
...
treeCV <- train(stars ~ . ,
                  data = df_train, 
                  method = "rpart", 
                  trControl = model, #esto cambia
                  tuneGrid = grid) # todos los valores de cp y escoge el mejor
...
metrics_train_df <- data.frame( model = c( "Linear Regression" ,
                          ...
                          MAE = c( MAE_train_LR, MAE_train_tree, MAE_train_treeCV )
                          )
...
metrics_test_df <- data.frame( model = c( "Linear Regression" ,
                                     "Decision tree",
                                     "Decision tree CV" ),
                          Rsquared = c( R2_test_LR, R2_test_tree ,R2_test_treeCV ),
                          MAE = c( MAE_test_LR, MAE_test_tree, MAE_test_treeCV )
                          )
...
Observación: cuando veo un y solito estoy haciendo un pronóstico

-   y_test_LR_bin = 1: Cuando y_test_LR \>=4

<!-- -->

-   y_test_tree_bin = 1: Cuando y_test_tree\>=4

-   y_test_treeCV_bin = 1: Cuando y_test_treeCV \>=4
...
NO, porque eso solo sirve para evaluar el modelo de regresión. Sin embargo para evaluar un modelo de clasificación usamos la especificidad, severidad, matriz de confusion. En si necesito métricas para mi modelo de clasificación
...
# GUARDAMOS LA CLASE MAYORITARIA EN TRAIN DATA
majorClass <- ifelse(mean(dfclass_train$aboveFour) >= 0.5, 1, 0)
...
-   Convierta enseguida y_test_baseline a factor con levels = c(1,0)

-   Convierta dfclass_test\$aboveFour a factores con levels = c(1,0)

```{r}
# COMENTARIO AQUI
y_test_baseline <- y_test_baseline <- rep(majorClass, times = nrow(dfclass_test))
...
```{r}
# Cambiamos a factor aboveFour con levels c(1,0).
dfclass_test$aboveFour <- factor( dfclass_test$aboveFour, levels = c(1, 0) )
dfclass_train$aboveFour <- factor( dfclass_train$aboveFour, levels = c(1, 0) )
...
y_train_reglog_bin <-  factor(y_train_reglog_bin, levels = c(1,0))
...
```{r}
# matriz de confusion: dfclass_test vs y_test_reglog_bin
y_test_reglog_bin <-  factor(y_test_reglog_bin, levels = c(1, 0))

matrixCM_test_reglog <- confusionMatrix(data = y_test_reglog_bin, 
                           reference = dfclass_test$aboveFour)
...
# Predecimos los valores en nuestros datos de prueba
...
y_train_pred_classTree <-  factor(y_train_pred_classTree, levels = c(1,0))

matrixCM_train_classTree <-  confusionMatrix(data = y_train_pred_classTree, 
                           reference = dfclass_train$aboveFour)
...
y_test_pred_classTree <-  factor(y_test_pred_classTree, levels = c(1,0))

matrixCM_test_classTree <- confusionMatrix(data = y_test_pred_classTree, 
                           reference = dfclass_test$aboveFour)
...
y_test_pred_classTreeCV <- predict(treeClassCV, dfclass_test)
```
...
```{r}
# COMENTE AQUI
y_train_pred_classTreeCV <-  factor(y_train_pred_classTreeCV, levels = c(1,0))

matrixCM_train_classTreeCV <- confusionMatrix(data = y_train_pred_classTreeCV, 
                           reference = dfclass_train$aboveFour)
...
```{r}
# COMENTE AQUI
y_test_pred_classTreeCV <-  factor(y_test_pred_classTreeCV, levels = c(1,0))
df_test$aboveFour <- factor(dfclass_test$aboveFour, levels = c(1, 0))


matrixCM_test_classTreeCV <- confusionMatrix(data = y_test_pred_classTreeCV, 
                                               reference = dfclass_test$aboveFour)
...
pred_rf <- predict( rfClass, dfclass_train)#data de entrenamiento

# Matriz de confusion entre: pred_rf vs dfclass_test$aboveFour                   
confusionMatrix( data = pred_rf , reference =  dfclass_test$aboveFour)
```
